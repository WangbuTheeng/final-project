...
<body>
    <!-- Watermark -->
    <div class="watermark">{{ $collegeSettings->name ?? 'BAJRA INTERNATIONAL COLLEGE' }}</div>

    <!-- Header -->
    <div class="header">
        <div class="college-name">{{ $collegeSettings->name ?? 'Bajra International College' }}</div>
        <div class="college-address">
            {{ $collegeSettings->address ?? 'Affiliated to Tribhuvan University' }}
            @if($collegeSettings->phone)
                <br>Phone: {{ $collegeSettings->phone }}
            @endif
            @if($collegeSettings->email)
                | Email: {{ $collegeSettings->email }}
            @endif
        </div>
    ...
    <div class="summary-section">
        <div class="summary-grid">
            <div class="summary-item">
                <div class="summary-value" style="color: #2563eb;">{{ number_format($totalObtained, 1) }}</div>
                <div class="summary-label">Total Obtained</div>
            </div>
            <div class="summary-item">
                <div class="summary-value" style="color: #1f2937;">{{ number_format($totalMaximum, 1) }}</div>
                <div class="summary-label">Total Maximum</div>
            </div>
            <div class="summary-item">
                <div class="summary-value" style="color: #7c3aed;">{{ number_format($overallPercentage, 2) }}%</div>
                <div class="summary-label">Overall Percentage</div>
            </div>
            <div class="summary-item">
                <div class="summary-value" style="color: #059669;">{{ number_format($gpa, 2) }}</div>
    ...
    <div class="result-declaration">
        <div class="result-status {{ $resultStatus == 'Pass' ? 'result-pass' : 'result-fail' }}">
            RESULT: {{ strtoupper($resultStatus) }}
        </div>
        <div style="font-size: 16px; font-weight: bold; color: #374151;">
            Overall Grade: {{ $overallGrade }} | GPA: {{ number_format($gpa, 2) }}
        </div>
    ...
    <div class="footer">
        ...
        <p>Generated on {{ now()->format('F d, Y \a\t g:i A') }} | {{ $collegeSettings->name ?? 'Bajra International College' }} Management System</p>
        <p>For verification, contact: {{ $collegeSettings->email ?? 'info@bajracollege.edu.np' }} | {{ $collegeSettings->phone ?? '+977-1-XXXXXXX' }}</p>
    </div>
</body>
</html>
